let userName = FavsHubSettings.get('userName') || '寰宇';
...
const WelcomeManager = {
    ...
    initialize() {
        ...
        if (welcomeElement) {
            welcomeElement.style.display = showWelcome !== false ? '' : 'none';
    ...
    updateWelcomeMessage(checkVisibility = true) {
        ...
        if (welcomeElement) {
            ...
            // 只有在需要时才检查显示状态
            if (checkVisibility) {
                welcomeElement.style.display = FavsHubSettings.get('showWelcomeMessage') !== false ? '' : 'none';
            }
    ...
    setupEventListeners() {
        document.getElementById('welcome-message').addEventListener('click', () => {
            ...
            if (newUserName && newUserName.trim() !== "") {
                ...
                FavsHubSettings.set('userName', userName);
