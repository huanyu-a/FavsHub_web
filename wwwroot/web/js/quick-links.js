...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function faviconURL(u) {
    ...
    if (window.getFaviconUrl) {
      return window.getFaviconUrl(u, 32);
    }
    // 备用方案
    try {
  ...
  function getFixedShortcuts() {
    return Promise.resolve(FavsHubSettings.get('fixedShortcuts') || []);
  }
  ...
  function updateFixedShortcut(updatedSite, oldUrl) {
    ...
    FavsHubSettings.set('fixedShortcuts', fixedShortcuts);
  ...
  // createQRCode 已在 script.js 中全局定义，此处直接调用

  // 获取黑名单
  function getBlacklist() {
    return Promise.resolve(FavsHubSettings.get('blacklist') || []);
  }

  // 添加到黑名单
  function addToBlacklist(domain) {
    const blacklist = FavsHubSettings.get('blacklist') || [];
    if (!blacklist.includes(domain)) {
      blacklist.push(domain);
      FavsHubSettings.set('blacklist', blacklist);
      return true;
    }
    return false;
  }
