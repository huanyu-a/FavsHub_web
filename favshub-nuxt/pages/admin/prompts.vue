    ...
    <div v-if="tab === 'folders'" class="card">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-else-if="pFolders.length === 0"><td colspan="5" class="empty-state">暂无数据</td></tr>
          <tr v-for="f in pFolders" :key="f.id">
            <td>{{ f.name }}</td><td>{{ f.parent_name || '-' }}</td><td>{{ f.username || f.user_id }}</td>
            <td>{{ f.prompt_count || 0 }}</td>
            <td class="actions">
              <button class="btn btn-ghost btn-sm" @click="renamePFolder(f)">重命名</button>
    ...
    <div v-if="editVisible" class="modal-overlay" @click.self="editVisible = false">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="fg"><label>文件夹</label><select v-model="ef.folder_id"><option :value="null">未分类</option><option v-for="f in pFolders" :key="f.id" :value="f.id">{{ f.name }}</option></select></div>
...
async function loadPrompts() {
  pLoading.value = true
  const r = await $fetch<any>(`/api/admin/prompts?page=${pPage.value}&limit=${pPageSize}`)
  prompts.value = r.prompts || []
  pTotal.value = r.total || 0
  pLoading.value = false
}
async function loadStats() {
  const d = await $fetch<any>('/api/admin/stats')
...
async function loadPFolders() { pfLoading.value = true; const d = await $fetch<{ folders: any[] }>('/api/admin/prompt-folders'); pFolders.value = d.folders || []; pfLoading.value = false }
async function createPFolder() { const n = prompt('文件夹名称'); if (n) { await $fetch('/api/admin/prompt-folders', { method: 'POST', body: { name: n } }); loadPFolders() } }
async function renamePFolder(f: any) { const n = prompt('新名称', f.name); if (n) { await $fetch(`/api/admin/prompt-folders/${f.id}`, { method: 'PUT', body: { name: n } }); loadPFolders() } }
async function delPFolder(f: any) { if (!confirm(`删除「${f.name}」？`)) return; await $fetch(`/api/admin/prompt-folders/${f.id}`, { method: 'DELETE' }); loadPFolders() }
...
async function loadTdk() { const d = await $fetch<any>('/api/tdk'); Object.assign(tdk, { title: d.title || '', description: d.description || '', keywords: d.keywords || '' }) }
...
async function saveEdit() { await $fetch(`/api/admin/prompts/${ef.id}`, { method: 'PUT', body: { ...ef } }); editVisible.value = false; loadPrompts() }
async function delPrompt(p: any) { if (!confirm(`删除「${p.title}」？`)) return; await $fetch(`/api/admin/prompts/${p.id}`, { method: 'DELETE' }); loadPrompts(); loadStats() }
...
.modal { background: #fff; border-radius: 12px; width: 90%; max-width: 550px; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
