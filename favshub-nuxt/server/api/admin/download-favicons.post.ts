/**
 * POST /api/admin/download-favicons — 批量下载未本地化的 favicon
 * 按 hostname 分组，每个域名只下载一次
 */
...
export default defineEventHandler(async (event) => {
  ...
  const bookmarks = db.prepare("SELECT id, url, icon FROM bookmarks WHERE icon NOT LIKE '/images/favicons/%' OR icon IS NULL").all() as any[]
  if (!bookmarks.length) {
    return { success: true, message: '所有书签图标已本地化', count: 0 }
  }
